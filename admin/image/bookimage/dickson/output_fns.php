...
foreach ($cat_array as $row)  {
    $url = "listbook.php?catid=".$row['CAT_ID'];
    $title = $row['CAT_NAME'];
    echo "<li>";
    ...
    foreach ($book_array as $row)
	 {
      ...
      if (@file_exists("admin/image/bookimage/".$row['IMG']."")) {
        $title = "<img src=\"admin/image/bookimage/".$row['IMG']."\"style=\"border: 1px solid black\"/>";
        do_html_url($url, $title);
      } else {
        echo "&nbsp;";
      }
...
function display_book_details($book) {
  ...
  if (is_array($book)) {
    ...
    if (@file_exists("admin/image/bookimage/".$book['IMG'].""))  
	 {
		 $image = "admin/image/bookimage/".$book['IMG']."";
      $size = GetImageSize($image);
      if(($size[0] > 0) && ($size[1] > 0)) {
        echo "<td><img src=\"admin/image/bookimage/".$book['IMG']."\"style=\"border: 1px solid black\"/></td>";
      }
    }
    echo "<td><ul>";
