...
// Create page object
$bookpost_edit = new cbookpost_edit();
$Page =& $bookpost_edit;

// Page init
$bookpost_edit->Page_Init();

// Page main
$bookpost_edit->Page_Main();
?>
...
// Create page object
var bookpost_edit = new ew_Page("bookpost_edit");

// page properties
bookpost_edit.PageID = "edit"; // page ID
bookpost_edit.FormID = "fbookpostedit"; // form ID
var EW_PAGE_ID = bookpost_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
bookpost_edit.ValidateForm = function(fobj) {
...
bookpost_edit.Form_CustomValidate =  
...
bookpost_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
bookpost_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $bookpost->TableCaption() ?><br><br>
<a href="<?php echo $bookpost->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$bookpost_edit->ShowMessage();
?>
<form name="fbookpostedit" id="fbookpostedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return bookpost_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="bookpost">
...
<?php if ($bookpost->bookid->Visible) { // bookid ?>
	<tr<?php echo $bookpost->bookid->RowAttributes ?>>
		...
		<td<?php echo $bookpost->bookid->CellAttributes() ?>><span id="el_bookid">
<div<?php echo $bookpost->bookid->ViewAttributes() ?>><?php echo $bookpost->bookid->EditValue ?></div><input type="hidden" name="x_bookid" id="x_bookid" value="<?php echo ew_HtmlEncode($bookpost->bookid->CurrentValue) ?>">
</span><?php echo $bookpost->bookid->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class cbookpost_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$bookpost->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$bookpost->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$bookpost->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$bookpost->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($bookpost->bookid->CurrentValue == "")
			$this->Page_Terminate("bookpostlist.php"); // Invalid key, return to list
		switch ($bookpost->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$bookpost->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$bookpost->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($bookpost->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// bookid
			$bookpost->bookid->EditCustomAttributes = "";
			$bookpost->bookid->EditValue = $bookpost->bookid->CurrentValue;
			$bookpost->bookid->CssStyle = "";
			$bookpost->bookid->CssClass = "";
			$bookpost->bookid->ViewCustomAttributes = "";
